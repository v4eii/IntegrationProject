...
public class EJBProxy {
	...
	public void log(DataObject input1) {
		...
		try {
			serializer.writeDataObject(input1, input1.getType().getURI(),
					input1.getType().getName(), outputStream);
	...
	/**
	 * Method generated to support implementation of operation "logReturn" defined for WSDL port type 
	 * named "EJBProxyImpl".
	 * 
	 * The presence of commonj.sdo.DataObject as the return type and/or as a parameter 
	 * type conveys that it is a complex type. Please refer to the WSDL Definition for more information 
	 * on the type of input, output and fault(s).
	 */
	public String logReturn(DataObject input1) {
		BOXMLSerializer serializer = (BOXMLSerializer) new ServiceManager()
				.locateService("com/ibm/websphere/bo/BOXMLSerializer");
		ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
		String xml = null;
	 ...
	 * for the operation "LoggerLocal#log(String arg0)"
	 * of wsdl interface "LoggerLocal"	
	 */
	public void onLogResponse(Ticket __ticket, DataObject returnValue,
			Exception exception) {
